...
- **Cycle 2**: ジョッキー要素を追加
- **Cycle 3**: 馬の実績要素を追加
- **Cycle 4**: 近走の勢いとJRA払戻率(80%)の正規化
- **Cycle 5**: コース適性と斤量増減を追加
- **Cycle 6**: 距離適性と馬場状態適性を追加
- **Cycle 7**: 直近2年のタイム指数と上がり3Fを追加
- **Cycle 8**: コンビ実績・乗り替わりを追加
- **Cycle 9**: 対数変換・パラメータ調整
- **Cycle 10**: 前走実績の直接投入 (着順・人気・オッズ)
"""
...
def fetch_horse_v10_stats():
    print("Fetching Cycle 10 comprehensive horse stats...")
    query = """
    ...
    df = df.with_columns([pl.col("running_time").map_elements(time_to_seconds, return_dtype=pl.Float64).alias("time_secs")])
    ...
    combo_stats = df.group_by(["horse_name", "jockey_name"]).agg([
        pl.col("finish_position").mean().alias("combo_avg_finish_pos")
    ])

    prev_race = df.group_by("horse_name").head(1).select([
        "horse_name", "finish_position", "popularity", "odds", "jockey_name", "handicap"
    ]).rename({
        "finish_position": "prev_finish_pos", "popularity": "prev_popularity",
        "odds": "prev_odds", "jockey_name": "prev_jockey_name", "handicap": "prev_handicap"
    })
    ...
    res = all_time.join(prev_race, on="horse_name", how="left").join(speed_stats, on="horse_name", how="left") \
                  .join(f600_stats, on="horse_name", how="left").join(nakayama, on="horse_name", how="left") \
                  .join(dist_1200, on="horse_name", how="left").join(firm_track, on="horse_name", how="left") \
                  .join(recent_3, on="horse_name", how="left")
    return res, combo_stats

def fetch_training_data_cycle_10(j_stats, h_stats, combo_stats):
    query = """
    ...
    """
    df = pl.read_database_uri(query, get_db_url())
    df = df.join(j_stats, on="jockey_name", how="left").join(h_stats, on="horse_name", how="left").join(combo_stats, on=["horse_name", "jockey_name"], how="left")
    ...
    fill_cols = [c for c in df.columns if c not in ["horse_number", "handicap", "age", "sex", "jockey_name", "horse_name", "odds", "prev_jockey_name"]]
    for c in fill_cols:
        m = df.select(pl.col(c).mean()).item()
        df = df.with_columns(pl.col(c).fill_null(m if m is not None else 0))
    return df
...
def train_cycle_10():
    ...
    h_stats, combo_stats = fetch_horse_v10_stats()
    train_df = fetch_training_data_cycle_10(j_stats, h_stats, combo_stats)
    ...
    features = [
        ...
        "combo_avg_finish_pos", "jockey_changed",
    ...
    print(f"Training Cycle 10 Final Model on {len(X)} records...")
    model = lgb.LGBMRegressor(n_estimators=250, learning_rate=0.03, num_leaves=40, random_state=42, verbosity=-1)
    ...
    for f in features:
        if f in ["horse_number", "handicap", "age", "sex_encoded", "handicap_diff", "jockey_changed"]: continue
