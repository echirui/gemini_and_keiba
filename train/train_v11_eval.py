"""
# Cycle 11: 過去レース(202506020111)を用いた誤差最小化チューニング
"""
...
def time_to_seconds(t_str):
    try:
        ...
        m, s = str(t_str).split(":"); return float(m) * 60 + float(s)
    except: return None

def fetch_stats_before_date(target_date):
    print(f"Fetching stats before {target_date}...")
    query = f"""
    ...
    h_combined = h_combined.join(firm_track, on="horse_name", how="left")
...
def train_cycle_11():
    ...
    j_stats, h_stats, combo_stats = fetch_stats_before_date(target_date)
    ...
    features = [
        "horse_number", "handicap", "age", "jockey_avg_odds", "jockey_win_rate",
    ...
    print(f"Training Cycle 11 tuned model on {len(X)} records...")
    model = lgb.LGBMRegressor(
        n_estimators=300, learning_rate=0.02, num_leaves=45, 
        lambda_l1=0.1, lambda_l2=0.1, random_state=42, verbosity=-1
    )
    model.fit(X, y_log_prob)
    
    actual_data = {
        1: 12.3, 2: 9.9, 3: 5.9, 4: 133.8, 5: 45.3, 6: 60.5, 7: 34.9, 8: 11.2, 
        9: 90.4, 10: 8.0, 11: 2.7, 12: 8.0, 13: 9.6, 14: 379.5, 15: 132.6
    }
    ...
    with open("target_race_2025.txt", "r") as f:
        for line in f:
            p = line.strip().split(",")
            sex, age = p[2][0], int(p[2][1:])
            target_df_list.append({"horse_number": int(p[0]), "horse_name": p[1], "sex": sex, "age": age, "handicap": float(p[3]), "jockey_name": p[4], "actual_odds": actual_data[int(p[0])]})
    
    target_df = pl.DataFrame(target_df_list)
    ...
    for f in features:
        if f in ["horse_number", "handicap", "age", "handicap_diff", "jockey_changed"]: continue
        m = train_df.select(pl.col(f).mean()).item()
    ...
    print("\n--- Tuning Results: Predicted vs Actual ---")
    ...
    print(f"\nModel Performance (Log-RMSE): {rmse:.4f}")
