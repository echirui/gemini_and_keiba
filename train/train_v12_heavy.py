"""
# Cycle 履歴
- Cycle 1-11: 基礎、ジョッキー、馬、適性、スピード、コンビ、対数変換、評価環境
- Cycle 12: 重賞実績（格）の追加
    - 特徴量: `horse_max_grade`, `horse_is_g1_performer`
"""
...
def fetch_stats_v12(target_date):
    print(f"Fetching stats before {target_date} with grades...")
    query = f"""
        SELECT h.name as horse_name, j.name as jockey_name, r.finish_position, r.popularity, r.odds, 
               ...
               r.running_time, r.final_600m_time, r.race_name, r.race_class
    ...
    speed_stats = recent_2y.with_columns(pl.col("distance").sub(1200).abs().alias("dist_diff"))
    speed_stats = speed_stats.sort(["horse_name", "dist_diff", "speed"], descending=[False, False, True]).group_by("horse_name").head(1)
    speed_stats = speed_stats.select(["horse_name", "speed"]).rename({"speed": "horse_recent_max_speed"})
...
def train_cycle_12():
    ...
    j_stats, h_stats, combo_stats = fetch_stats_v12(target_date)
    ...
    print(f"Training Cycle 12 heavy model on {len(X)} records...")
    model = lgb.LGBMRegressor(n_estimators=400, learning_rate=0.02, num_leaves=50, random_state=42, verbosity=-1)
    model.fit(X, y_log_prob)
    
    actual_data = {1: 12.3, 2: 9.9, 3: 5.9, 4: 133.8, 5: 45.3, 6: 60.5, 7: 34.9, 8: 11.2, 9: 90.4, 10: 8.0, 11: 2.7, 12: 8.0, 13: 9.6, 14: 379.5, 15: 132.6}
    target_df_list = []
    ...
    print("\n--- Tuning Results (Cycle 12: Heavy stats added) ---")
