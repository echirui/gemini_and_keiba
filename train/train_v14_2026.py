"""
# Cycle 14: 2026年ターゲットレース最終予測 (永続化対応版)
"""
...
def predict_2026():
    ...
    raw_probs = np.exp(model.predict(X_target))
    norm_probs = (np.power(raw_probs, 1.5) / np.power(raw_probs, 1.5).sum()) * 0.8 # v14は1.5乗のまま
    target_df = target_df.with_columns(pl.Series("predicted_odds", 0.8 / norm_probs))
    print("--- 2026 Results (v14 Persist) ---")
