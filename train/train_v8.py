...
def fetch_horse_v8_stats():
    print("Fetching v8 horse & combo stats...")
    query = """
    ...
    last_jockey = df.group_by("horse_name").head(1).select(["horse_name", "jockey_name", "handicap"]).rename({
        "jockey_name": "prev_jockey_name",
        "handicap": "prev_handicap"
    })
    ...
    res = res.join(last_jockey, on="horse_name", how="left")
    return res, combo_stats

def fetch_training_data_cycle_8(j_stats, h_stats, combo_stats):
    query = """
    ...
    """
    df = pl.read_database_uri(query, get_db_url())
    df = df.join(j_stats, on="jockey_name", how="left")
    df = df.join(h_stats, on="horse_name", how="left")
    df = df.join(combo_stats, on=["horse_name", "jockey_name"], how="left")
    
    df = df.with_columns([
        pl.lit(0.0).alias("handicap_diff"),
...
def train_cycle_8():
    ...
    h_stats, combo_stats = fetch_horse_v8_stats()
    train_df = fetch_training_data_cycle_8(j_stats, h_stats, combo_stats)
    ...
    print(f"Training Cycle 8 on {len(X)} records...")
    ...
    print("\n--- Prediction Results (Cycle 8) ---")
    print(target_df.select(["horse_number", "horse_name", "jockey_name", "jockey_changed", "predicted_odds"]).sort("predicted_odds"))
