...
- **Cycle 8**: ジョッキーと馬のコンビ実績・乗り替わりを追加
- **Cycle 9**: 学習アルゴリズムの高度化 (対数変換・パラメータ調整)
"""
...
def fetch_horse_v9_stats():
    print("Fetching v9 horse & combo stats...")
    query = """
    ...
    last_info = df.group_by("horse_name").head(1).select(["horse_name", "jockey_name", "handicap"]).rename({
        "jockey_name": "current_last_jockey",
        "handicap": "current_last_handicap"
    })
    ...
    res = res.join(last_info, on="horse_name", how="left")
    return res, combo_stats

def fetch_training_data_cycle_9(j_stats, h_stats, combo_stats):
    query = """
    ...
    df = df.with_columns([
        (pl.col("handicap") - pl.col("current_last_handicap")).fill_null(0.0).alias("handicap_diff"),
        pl.when(pl.col("jockey_name") == pl.col("current_last_jockey")).then(0).otherwise(1).alias("jockey_changed")
    ])
    
    fill_cols = [c for c in df.columns if c not in ["horse_number", "handicap", "age", "sex", "jockey_name", "horse_name", "odds", "current_last_jockey"]]
    for c in fill_cols:
...
def train_cycle_9():
    ...
    h_stats, combo_stats = fetch_horse_v9_stats()
    train_df = fetch_training_data_cycle_9(j_stats, h_stats, combo_stats)
    ...
    print(f"Training Cycle 9 on {len(X)} records...")
    model = lgb.LGBMRegressor(n_estimators=200, learning_rate=0.03, num_leaves=31, random_state=42, verbosity=-1)
    ...
    print("\n--- Prediction Results (Cycle 9: Log-Prob & Tuned) ---")
